...
- **Hand-built API reference.** `/docs` is rendered by the Worker in the same design system instead of a third-party Swagger bundle, with server-side syntax highlighting for shell, JSON and raw HTTP, and the raw OpenAPI 3.1 document at `/openapi.json`.
- **WebGL hero.** Drifting topographic contours built on [webgl-noise](https://github.com/ashima/webgl-noise) (simplex noise by Ian McEwan and Stefan Gustavson, Ashima Arts, MIT). Decorative and defensive: no WebGL, a lost context or a failed compile all leave the page reading exactly as it does without it, and `prefers-reduced-motion` gets a single static frame.
